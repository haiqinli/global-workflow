...
#JKHtaskplan = ['getic', 'fv3ic', 'waveinit', 'waveprep', 'fcst', 'post', 'wavepostsbs', 'wavepostbndpnt', 'wavepostbndpntbll', 'wavepostpnt', 'wavegempak', 'waveawipsbulls', 'waveawipsgridded', 'wafs', 'wafsgrib2', 'wafsblending', 'wafsgcip', 'wafsgrib20p25', 'wafsblending0p25', 'vrfy', 'metp', 'arch']
taskplan = ['wsaeroic']
...
def get_definitions(base):
    ...
    strings.append('\t<!ENTITY WSICSDIR "%s">\n' % base['WSICSDIR'])
    strings.append('\t<!ENTITY FIXDIR "%s">\n' % base['FIXDIR'])
    strings.append('\n')
...
#JKH    envars.append(rocoto.create_envar(name='PDY', value='<cyclestr>@Y@m@d</cyclestr>'))
#JKH    envars.append(rocoto.create_envar(name='cyc', value='<cyclestr>@H</cyclestr>'))
    ...
    # wsaeroic
    deps = []
    data = '&WSICSDIR;/&CDUMP;.@Y@m@d/@H/</cyclestr>/RESTART/<cyclestr>@Y@m@d.@H0000.fv_tracer.res.tile6.nc'
    ...
    wsics_dir = rocoto.create_envar(name='WSICS_DIR', value='<cyclestr offset="-06:00:00">&WSICSDIR;/&CDUMP;.@Y@m@d/@H/RESTART/</cyclestr>')
    fixfv3 = rocoto.create_envar(name='FIXfv3', value='&FIXDIR;/fix_fv3')
    aeroenvars = envars + [ROTDIR] + [case] + [wsics_dir] + [fixfv3]

    task = wfu.create_wf_task('wsaeroic', cdump=cdump, envar=aeroenvars, dependency=dependencies)
    tasks.append(task)
    tasks.append('\n')
