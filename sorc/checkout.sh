...
if [[ ! -d fv3gfs.fd ]] ; then
    ...
    if [ ${run_ccpp:-"NO"} = "NO" ]; then
        ...
        if [[ ! -d ufs-weather-model_15dec_dbdb629 ]] ; then
            ...
            git clone --recursive -b gsl/develop https://github.com/NOAA-GSL/ufs-weather-model ufs-weather-model_15dec_dbdb629 >> ${topdir}/checkout-15dec.log 2>&1
    ...
    if [ ${run_ccpp:-"NO"} = "YES" ]; then
      ...
      rsync -avx fv3gfs.fd_gsl/ fv3gfs.fd/        ## copy over changes not in FV3 repository
    fi
else
...
if [[ ! -d ufs_utils.fd ]] ; then
    ...
    cd ${topdir}
#JKH    rsync -avx ufs_utils.fd_gsl/ ufs_utils.fd/        ## copy over changes not in UFS_UTILS repository
else
...
if [[ ! -d gfs_post.fd ]] ; then
    ...
    cd ${topdir}
#JKH    rsync -avx gfs_post.fd_gsl/ gfs_post.fd/        ## copy over GSL changes not in EMC_post repository
else
