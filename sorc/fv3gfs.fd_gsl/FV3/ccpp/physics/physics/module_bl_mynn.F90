...
!     **  Limitation on c3sq (0.12 =< cw =< 0.76) **
          ...
          auh = 27.*a1*((a2*a2fac)**2)*b2*(grav/tref)**2
          aum = 54.*(a1**2)*(a2*a2fac)*b2*c1*(grav/tref)
          adh = 9.*a1*((a2*a2fac)**2)*(12.*a1 + 3.*b2)*(grav/tref)**2
          adm = 18.*(a1**2)*(a2*a2fac)*(b2 - 3.*(a2*a2fac))*(grav/tref)
...
!test to limit unreasonably large negative fluxes:
   qvflux = flqv
    if (qvflux < 0.0) then
       !do not flux out more than 80% of moisture in a single dt
       !qvflux = max(qvflux, (-1.0*max(0.8*sqv(k),1e-8)  /dtz(k)))
       !do not allow flux to reduce sfc qv below 1e-8 kg/kg
...
!    d(k)=sqv(k)  + dtz(k)*flqv + qcd(k)*delt - dtz(k)*rhoinv(k)*s_awqv(k+1) - dtz(k)*rhoinv(k)*sd_awqv(k+1) + &
    d(k)=sqv(k)  + dtz(k)*qvflux + qcd(k)*delt - dtz(k)*rhoinv(k)*s_awqv(k+1) - dtz(k)*rhoinv(k)*sd_awqv(k+1) + &
       & sub_sqv(k)*delt + det_sqv(k)*delt
...
!       acfac = .5*tanh((fltv - 0.02)/0.09) + .5 
    ...
    wstar=max(1.E-2,(gtr*fltv*pblh)**(1./3.))
...
!            ENT(k,i)=real(ENTi(k,i))*Ent0/(ZW(k+1)-ZW(k))
      ...
      wstar   = max(0.,(grav/thv(1)*wthv*pblh)**(1./3.))
